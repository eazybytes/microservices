...
`loans`, `cards`, `gatewayserver`), parameterized so the same `docker-compose.yml` runs local,
...
| File                 | Purpose                                                              |
|----------------------|-----------------------------------------------------------------------|
| `docker-compose.yml` | Service definitions for `configserver`, `eurekaserver`, `accounts`, `loans`, `cards`, `gatewayserver`. |
...
```yaml
  configserver:
    image: "eazybytes/configserver:${IMAGE_TAG:-s9}"
    container_name: configserver-ms
    ports:
      - "8071:8071"
```
- `${IMAGE_TAG:-s9}` — image tag comes from the env file, defaulting to `s9` if unset. This is
...
```yaml
  eurekaserver:
    image: "eazybytes/eurekaserver:${IMAGE_TAG:-s9}"
...
```yaml
  accounts:
    image: "eazybytes/accounts:${IMAGE_TAG:-s9}"
...
```yaml
  gatewayserver:
    image: "eazybytes/gatewayserver:${IMAGE_TAG:-s9}"
...
downstream services (`accounts`, `loans`, `cards`) to report healthy before starting, and extends
`microservice-eureka-config` so it can route to them by discovering their instances through
Eureka rather than hardcoded hostnames. Declares no healthcheck of its own.
...
|-------------------|-----------------|-------------------|---------------------|
| `IMAGE_TAG`      | `s9`            | pin a tested tag  | pin a tested tag    |
