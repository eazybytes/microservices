...
`eurekaserver`, `accounts`, `loans`, `cards`, `gatewayserver`) with one command, instead of
running `mvn compile jib:dockerBuild` and `docker image push ...` by hand in each module
directory.
...
(`configserver/`, `eurekaserver/`, `accounts/`, `loans/`, `cards/`, `gatewayserver/`) must stay
one level above `build-scripts/` at the repo root — you don't need to run the scripts from any
...
```bash
./build-scripts/build-images.sh                    # build all 6 services, one at a time
./build-scripts/build-images.sh accounts cards      # build only the named services
./build-scripts/build-images.sh --parallel          # build all 6 services concurrently
./build-scripts/build-images.sh -p accounts cards   # build named services concurrently
./build-scripts/build-images.sh --push              # build all 6 locally, then push each to Docker Hub
...
- **No arguments**: builds all six services, `configserver eurekaserver accounts loans cards gatewayserver`.
...
- **`--push`**: after each service builds successfully into the local Docker daemon (same
  ...
  currently `eazybytes/<service>:s10` on Docker Hub) rather than hardcoded here, so it can't
